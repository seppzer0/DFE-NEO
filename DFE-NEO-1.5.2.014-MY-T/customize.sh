...
my_print "You want to patch external boot?"
MYSELECT "YES" "NO"
if [ $? = 1 ] ; then 

find /sdcard/ \
-name "*.img" -and -size +6MB >> $tmp/list
#-name "*.zip" -size +6MB \
#-or \

all_line=$(
wc -l $tmp/list | awk '{print $1}'
)

tick=1
while (( $tick != $(( $all_line + 1 )) )) ; do
echo "$tick) $(cat $tmp/list | head -n$tick | tail -n1 | sed 's|/sdcard/||')"
tick=$(( tick + 1 ))
done
A=1
while true; do
        DIRSsell=$(cat $tmp/list | head -n$A | tail -n1 | sed 's|/sdcard/||')
        echo " > $(echo "${DIRSsell}" | sed 's/full-// ; s/apps-//') <"
        if chooseport 60; then
            A=$((A + 1))
        else
            break
...
echo " "
echo "Selected \"$(cat $tmp/list | head -n$A | tail -n1)\""
ex_boot="$(cat $tmp/list | head -n$A | tail -n1)"
exboot=true
...
for forslot in $(
    ...
    "lib/$CPU/libmagisk32.so" -j -d $tmp_boot &>$(dirname $arg3)/log.txt
     ...
     my_print "$text113 ($(basename $boot)) $($A_only || echo $text78 $forslot)"
            ...
            ! $exboot && flash_image ./new-boot.img $boot
            $exboot && cp ./new-boot.img /sdcard/new-boot_dfe.neo.img
